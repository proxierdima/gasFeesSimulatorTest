...
def parse_constructor_args(raw: str):
    raw = (raw or "").strip()
    if not raw:
        return []
    try:
...
def main() -> int:
    ...
    parser.add_argument("--constructor-args", default="[]")
    parser.add_argument("--gaslimit", default="")
    args = parser.parse_args()

    ctor_args = parse_constructor_args(args.constructor_args)

    payload = run_backend_template(
        "GLH_SUBMIT_DEPLOY_BACKEND_CMD",
        {
            "rpc_url": args.rpc,
            "contract_file": args.contract_file,
            "args_json": json.dumps(ctor_args, ensure_ascii=False),
